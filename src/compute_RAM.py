...
# --- Ваши функции preprocess_audio, text_to_int, char_map ---
...
def preprocess_audio(audio_path, sample_rate=16000, n_mels=80, n_fft=400, hop_length=160):
    # ... (ваш код preprocess_audio без изменений, возвращает тензор (T, F)) ...
    try:
        audio, orig_sr = librosa.load(audio_path, sr=sample_rate, mono=True)
    except Exception as e:
        # print(f"Ошибка загрузки файла {audio_path}: {e}") # Можно отключить для скорости
        return None
...
for index, row in tqdm(audio_target.iterrows(), total=len(audio_target)):
    ...
    if not os.path.exists(potential_path_opus):
        # print(f"Файл не найден: {potential_path_opus}") # Можно отключить
        continue
...
try:
    ...
    # !!! Эта оценка может быть неточной, реальное потребление может быть выше !!!
except ImportError:
